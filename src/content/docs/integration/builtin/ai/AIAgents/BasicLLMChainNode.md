...
- Simple prompt-response patterns for text generation, analysis, and transformation tasks
- Browser context-aware AI processing with web page data integration

### Key Features

- **Direct LLM Integration**: Connect to multiple LLM providers including OpenAI, Anthropic, and local models with minimal configuration
...
| `prompt` | `string` | The prompt template with variable 
...
This example demonstrates the fundamental usage of the BasicLLMChainNode node in a typical workflow scenario.
...
1. **Previous Node** → **BasicLLMChainNode** → **Next Node**
