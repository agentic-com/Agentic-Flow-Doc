...
1. **Add content source**: Drag a "Get All Text from Link" node to extract text from any webpage
...
```mermaid
graph LR
    URL[Web Page URL] --> Extract[Get All Text from Link]
...
**Components used:**
- Get All Text from Link (content extraction)
...
2. **Extract:** Get All Text from Link node
...
**Try workflow patterns:** Check out [Workflow Patterns](./workflow-patterns) for proven approaches to common tasks

**Browser integration:** Learn how to combine AI with web automation in [Browser Integration](./browser-integration-guide)
