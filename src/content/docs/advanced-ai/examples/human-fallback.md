...
This workflow uses the [Chat Trigger](/integrations/builtin/core-nodes/Agentic Workflow Studio-nodes-langchain.chattrigger/index.md) to provide the chat interface, and the Call Agentic Workflow Studio Workflow Tool to call a second workflow that handles checking for email addresses and sending the Slack message.
...
* Call Agentic Workflow Studio Workflow Tool: plug in Agentic Workflow Studio workflows as custom tools. In AI, a tool is an interface the AI can use to interact with the world (in this case, the data provided by your workflow). It allows the AI model to access information beyond its built-in dataset.
