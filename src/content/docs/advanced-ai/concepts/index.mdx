...
<CardGrid>
  ...
  <Card title="Action Approval" icon="approve-check">
    Page content is untrusted input. When a model's output drives a click, a request, or an MCP call, a user approval gate stands in the way.

    [Understand action approval →](./action-approval)
  </Card>
...
| Let an agent take actions | Tools Agent + action tools | Action approval, Browser / HTTP Request / MCP Client tools |
