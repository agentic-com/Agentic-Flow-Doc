...
Slack belongs to the **Integration** palette group. Its canonical node id is `slack` and its source documentation link is `/nodes/builtin/integration/slack/`.

## When to use it

Use Slack when the workflow needs to call this external service with configured credentials.
