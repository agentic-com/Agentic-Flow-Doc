...
description: "Get all the text content from a given URL"
sidebar:
...
Get all the text content from a given URL

## What it does

Get All Text from Link belongs to the **Core** palette group. Its canonical node id is `getAllTextFromLinkNode` and its source documentation link is `/nodes/builtin/core/getalltextfromlink/`.

## When to use it

Use Get All Text from Link for generic workflow operations such as requests, code, or URL content retrieval.
...
Connect data into Get All Text from Link, configure the settings above, and use the output in the next workflow step.
