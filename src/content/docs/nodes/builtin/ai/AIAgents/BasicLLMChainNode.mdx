...
Basic LLM Chain belongs to the **AI Agents** palette group. Its canonical node id is `basicLLMChain` and its source documentation link is `/nodes/builtin/ai/aiagents/basicllmchainnode/`.

## When to use it

Use Basic LLM Chain as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
