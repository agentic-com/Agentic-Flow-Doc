...
Web LLM belongs to the **Chat LLM** palette group. Its canonical node id is `webLLM` and its source documentation link is `/nodes/builtin/ai/aidependencies/llm/wbellm/`.

## When to use it

Use Web LLM as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
