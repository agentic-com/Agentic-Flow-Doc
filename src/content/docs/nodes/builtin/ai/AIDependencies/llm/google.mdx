---
title: "Chat Google"
description: "Leverage Google to use LLM locally directly on your device"
sidebar:
  label: "Chat Google"
---
...
Leverage Google to use LLM locally directly on your device

## What it does

Chat Google belongs to the **Chat LLM** palette group. Its canonical node id is `chat-google` and its source documentation link is `/nodes/builtin/ai/aidependencies/llm/google/`.

## When to use it

Use Chat Google as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
...
| Platform Type | Source-backed field from the node schema. |
...
Connect Chat Google to an AI agent or dependency input that accepts this dependency type, then run the agent with data from previous nodes.
