...
Chat OpenAI belongs to the **Chat LLM** palette group. Its canonical node id is `chat-openai` and its source documentation link is `/nodes/builtin/ai/aidependencies/llm/openai/`.

## When to use it

Use Chat OpenAI as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
