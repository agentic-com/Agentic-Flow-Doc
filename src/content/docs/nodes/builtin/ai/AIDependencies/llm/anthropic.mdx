...
description: "Leverage Anthropic to use LLM locally directly on your device"
sidebar:
...
Leverage Anthropic to use LLM locally directly on your device

## What it does

Chat Anthropic belongs to the **Chat LLM** palette group. Its canonical node id is `chat-anthropic` and its source documentation link is `/nodes/builtin/ai/aidependencies/llm/anthropic/`.

## When to use it

Use Chat Anthropic as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
...
Connect Chat Anthropic to an AI agent or dependency input that accepts this dependency type, then run the agent with data from previous nodes.
