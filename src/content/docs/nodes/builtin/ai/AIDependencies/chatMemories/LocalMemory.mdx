...
Local Memory belongs to the **Chat Memories** palette group. Its canonical node id is `localMemory` and its source documentation link is `/nodes/builtin/ai/aidependencies/chatmemories/localmemory/`.

## When to use it

Use Local Memory as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
