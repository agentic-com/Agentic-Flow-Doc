...
description: "Leverage OLLAM to use Embeddings locally directly on your device"
sidebar:
...
Leverage OLLAM to use Embeddings locally directly on your device

## What it does

Ollama Embeddings belongs to the **Embeddings** palette group. Its canonical node id is `ollamaEmbeddings` and its source documentation link is `/nodes/builtin/ai/aidependencies/embeddings/ollamaembeddings/`.

## When to use it

Use Ollama Embeddings as part of an AI workflow, either as an agent node or as a dependency connected to an agent.
...
Connect Ollama Embeddings to an AI agent or dependency input that accepts this dependency type, then run the agent with data from previous nodes.
