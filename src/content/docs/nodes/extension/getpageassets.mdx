...
Get Page Assets belongs to the **In Page Action** palette group. Its canonical node id is `getPageAssetsNode` and its source documentation link is `/nodes/extension/getpageassets/`.

## When to use it

Use Get Page Assets when the workflow needs to read from, write to, or interact with the currently open browser page.
