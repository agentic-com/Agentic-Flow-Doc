...
| OAuth2                           | Client ID (Client Secret only for confidential "Web application" clients), Redirect URL | Make sure tokens are valid and scopes are granted |
...
:::note[Client Secret storage]
Some integrations use a **public OAuth client** (PKCE-only) — for example **Microsoft 365 / Microsoft Graph**. Public clients never send a client secret to the token endpoint, so no client secret is stored for them: the field is hidden, and any secret from an older connection is cleared the next time you save the credential.

**Google** integrations (Gmail, Drive, Sheets, Calendar) remain a **confidential client**: a Google "Web application" OAuth client still requires its client secret at the token endpoint even with PKCE, so Google credentials keep their secret. (A Google "Desktop app" client is public and needs no secret — leave the field blank.)
:::
