...
- **Production execution** runs automatically in response to a trigger (schedule, event, etc.).  
  ...
  - A run that reaches a [Wait For Approval](/nodes/builtin/flow/wait-for-approval/) node pauses and waits for a human decision — it can be approved, rejected, or cancelled from the Executions pane rather than finishing immediately.
