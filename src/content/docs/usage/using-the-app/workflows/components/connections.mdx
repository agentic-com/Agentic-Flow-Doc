...
## Named outputs on branching nodes

Branching nodes — [If](/nodes/builtin/flow/if/) and [Switch](/nodes/builtin/flow/switch/) — expose more than one output port, and each port has a **name** (the branch or rule it routes to). When you connect a branch, the edge attaches to that specific named port.
