...
An individual node does not have its own separate "error" output. To run steps when something goes wrong, wrap the risky work in a **Try / Catch** node.
...
To react when an **entire workflow** fails (rather than a single step inside it), use an **Error Trigger** — a workflow that runs whenever a chosen workflow errors.
