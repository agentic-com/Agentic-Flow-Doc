    ...
    async fixOverviewSections() {
        ...
        if (fs.existsSync(nodeTypesPath)) {
            ...
            if (!content.includes('## Overview') && !content.includes('## Purpose')) {
                ...
                if (frontmatterEnd > 0) {
                    lines.splice(frontmatterEnd + 1, 0, '', '## Overview', '', 'This document provides a comprehensive guide to the different types of nodes available in Agentic Flow, helping you understand their categories, purposes, and when to use each type in your workflows.');
                    content = lines.join('\n');
        ...
        if (fs.existsSync(rateLimitsPath)) {
            ...
            if (!content.includes('## Overview') && !content.includes('## Purpose')) {
                ...
                if (frontmatterEnd > 0) {
                    lines.splice(frontmatterEnd + 1, 0, '', '## Overview', '', 'This document explains the rate limiting considerations and best practices for builtin nodes in Agentic Flow, helping you understand how to optimize your workflows for performance and reliability.');
                    content = lines.join('\n');
